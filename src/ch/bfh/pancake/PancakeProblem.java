...
public class PancakeProblem {
    ...
    public static void main(String... args) {
        ...
        String stackType = cmd.getOptionValue("stackType");
        String length = cmd.getOptionValue("length");
        int pancakeStackSize = 0;
        try {
            pancakeStackSize = Integer.parseInt(length);
        } catch (Exception e) {
            pancakeStackSize = 30;
        }
        ...
        int[] pancakes;
        if ("pairwise".equals(stackType)) {
            if ("count".equalsIgnoreCase(mode)) {
                pancakes = new int[]{2, 1, 4, 3, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 16, 15, 17};
            } else {
                pancakes = new int[]{2, 1, 4, 3, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 15};
            }
        } else {
            pancakes = ThreadLocalRandom.current().ints(1, pancakeStackSize).distinct().limit(pancakeStackSize - 1).toArray();
        }
    ...
    private static int findBestSolution(int[] pancakes, int bound) {
        ...
        pancakeStack.addFirst(new PancakeNode(pancakes, 0, 0, bound, new int[pancakes.length * 2]));
        int count = 0;
    ...
    private static Options createOptions() {
        ...
        Option length = new Option("l", "length", true, "length of the random array");
